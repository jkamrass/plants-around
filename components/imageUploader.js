...
export default function ImageUploader () {
  ...
  const uploadImage = () => {
    ...
    axios.post(`https://api.cloudinary.com/v1_1/dk4lsu1uf/image/upload`, data)
      .then(response => {
        debugger;
      })
      .catch(err => {
      ...
      <div className='col-sm-6 offset-md-3'>
        <div className="mb-3">
          ...
          <div className="input-group" id="fileInput">
            ...
            <button className="btn btn-outline-secondary" type="button" onClick={uploadImage}>Add Photo</button>
          </div>
        </div>
      </div>
      <div className="col-sm-2">
        {previewImage ? <img src={previewImage} alt='' className="img-thumbnail"/> : null}
      </div>
      </div>
    </>
  )
